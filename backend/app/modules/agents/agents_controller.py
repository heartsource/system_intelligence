...
async def fetch_agents(body: AgentListModel, response: Response):
    try:
        ...
        if agentsListData['data'] == [] or agentsListData['data'] == None:
            response.status_code = status.HTTP_404_NOT_FOUND
            return { "status": "error", "data": ERROR_MESSAGES.NOT_FOUND_ERROR }
        response.status_code = status.HTTP_200_OK
        return { "status": "success", "data": agentsListData['data'], "totalRecords": agentsListData['length'] }
...
async def create_agent(body: AgentModel, response: Response):
    try:
        ...
        return { "status": "success", "data": SUCCESS_MESSAGES.AGENT_CREATED_SUCCESS }   
    except Exception as e:
        return internalServerError(e, response)
...
async def update_agent(id: str, body: AgentUpdateModel, response: Response):
    try:
        ...
        response.status_code = status.HTTP_200_OK
        return { "status": "success", "data": SUCCESS_MESSAGES.AGENT_UPDATED_SUCCESS }
    except Exception as e:
        return internalServerError(e, response)
...
async def delete_agent(id: str, response: Response):
    try:
        ...
        response.status_code = status.HTTP_200_OK
        return { "status": "success", "data": SUCCESS_MESSAGES.AGENT_DELETED_SUCCESS }
    except Exception as e:
        return internalServerError(e, response)
